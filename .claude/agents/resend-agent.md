...
- `lib/email/resend-client.ts` — a lazily-constructed `Resend` client (`getResendClient()`), and `getEmailFrom()` which defaults to `Cuppa Cards <updates@cuppacards.com>` (overridable via `EMAIL_FROM`). That address must stay a domain verified in the Resend dashboard — sends fail otherwise.
