 ...
 * top-of-page Active Batches panel (components/submit/active-batches-panel.tsx).
 * No new API surface needed: supabase/migrations/0035_submission_pools.sql
 * already grants `pools_select_all` (using (true)) since pools carry no
 * customer PII, the same RLS grant components/PoolTracker.tsx's client-side
 * realtime view already relies on -- this just reads the same table
 * server-side for a one-shot render instead of a live-updating widget.
 *
