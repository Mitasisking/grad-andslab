...
export default async function AdminEventsPage() {
  ...
  const { data: settings } = await supabase.from('event_settings').select('active_event_slug, is_live').eq('id', true).single()
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      <EventSettingsPanel initialSettings={(settings ?? { active_event_slug: null, is_live: false }) as EventSettingsRow} />
    </main>
  )
}
