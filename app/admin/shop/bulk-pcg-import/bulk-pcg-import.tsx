...
/**
 * PCG's own published grading scale (https://pcgpopreport.com), unlike the
 * half-guessed ACE list in bulk-ace-import.tsx: the top of the scale carries
 * PCG's actual named tiers (Flawless 10, Pristine 10, Gem Mint 10, Gem Mint
 * 9.5), then plain numeric grades below that. DEFAULT_GRADE seeds every new
 * row so admins only need to change it for the (rare) card that isn't a
 * straight Gem Mint 10.
 */
const GRADE_OPTIONS = [
  'Flawless 10',
  'Pristine 10',
...
const DEFAULT_GRADE = 'Gem Mint 10'
...
/**
 * Manual-entry batch tool, not an auto-fetch one -- there's no PCG scraper
 * to fetch from (a real one would mean copying PCG's own slab images/grade
 ...
 * PCG's own live record -- see components/dashboard/cert-link.tsx). This
 ...
 * PCG's real site -- and, for Pokémon rows, the same TCGdex search used in
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <p className="text-[13.5px] mt-2 max-w-xl" style={{ color: 'var(--ink-muted)' }}>
        Paste PCG cert numbers, then for each row search for the card (Pokémon rows pull the official title, set, and
        image straight from TCGdex — Sports/General rows still take manual details), pick a grade, and cross-check
        against PCG&apos;s own verification page (the link on each row opens it) before saving. Type a real price and
            ...
            {rows.map((row) => (
              <div key={row.certNumber} className="border rounded-[3px] p-4" style={{ borderColor: 'var(--line)' }}>
                <div className="flex items-center justify-between gap-2">
                  <CertLink grader="PCG" certNumber={row.certNumber} />
                  {row.status === 'saved' && (
