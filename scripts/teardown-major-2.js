#!/usr/bin/env node
/**
 * Companion teardown for scripts/simulate-major-test-2.js — wipes every
 * [TEST-MAJOR-2] user and everything it created, including the two
 * dedicated throwaway shop products the simulation makes for itself.
 *
 * Written as a plain .js CommonJS script, matching every other script in
 * this scripts/ directory (teardown-major.js, teardown-minor-test.js) —
 * this project has no tsx/ts-node runner installed, so a .ts script here
 * would not actually be runnable with `node` without adding a new
 * dependency for a single throwaway script.
 *
 * Same explicit-delete discipline as teardown-major.js (see that file's
 * header): production's profiles table has none of the on-delete-cascade
 * FKs the migration files declare, so every child row (orders, order_items,
 * submissions, submission_items, addresses, ledger_entries) is deleted here
 * explicitly by id/user_id rather than trusted to cascade from deleting the
 * auth user.
 *
 * Precise path: reads scripts/.major-test-2-manifest.json (written by
 * simulate-major-test-2.js). Safety-net path: independently pages through
 * Supabase Auth for any user whose email ends in "@testmajor2.invalid" that
 * isn't already in the manifest, and a final prefix-scoped sweep for
 * [TEST-MAJOR-2] products/submissions/addresses that survived some other
 * way. Never a bare wildcard delete of a whole table.
 *
 * Usage:
 *   node scripts/teardown-major-2.js
 *
 * Requires the same .env.local as simulate-major-test-2.js.
 */
...
const PREFIX = '[TEST-MAJOR-2]'
const EMAIL_DOMAIN = 'testmajor2.invalid'
const MANIFEST_PATH = path.join(__dirname, '.major-test-2-manifest.json')
...
async function main() {
  ...
  if (fs.existsSync(MANIFEST_PATH)) {
    ...
    console.log('No manifest file found — falling back to a live sweep for [TEST-MAJOR-2] users/products only.')
  }
  ...
  if (targets.size === 0) {
    console.log('No [TEST-MAJOR-2] users found. Nothing to tear down.')
  }
  ...
  // order/order_item referencing them is gone.
  ...
  // [TEST-MAJOR-2] marker but not tied to a user handled above.
  ...
  if (orphanSubs?.length) {
    ...
    console.log(`\nRemoved ${orphanSubs.length} orphaned [TEST-MAJOR-2] submission(s).`)
  }
  ...
  if (orphanAddresses?.length) {
    ...
    console.log(`Removed ${orphanAddresses.length} orphaned [TEST-MAJOR-2] address(es).`)
  }
