...
import type { PrintStatus, ShopFilterState } from './product-filters'
...
// Launch rollout: Graded inventory is ACE-only, so there's no grader to
// choose between anymore -- kept for when PCG/PSA come back, see the
// commented-out render branch below.
...
export function SubcategoryPills({ activeCategory, filters, onChange }: Props) {
  // Launch rollout: Graded inventory is ACE-only (app/shop/page.tsx's query
  // now filters grading_company itself), so there's no grader to choose
  // between anymore -- this row is hidden rather than deleted; GRADER_OPTIONS
  // and the toggle logic below are untouched for when PCG/PSA come back.
